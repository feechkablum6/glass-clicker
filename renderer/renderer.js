...
function setWelcome(visible) {
    ...
    if (visible) {
        ui.welcome.classList.remove('leaving');
        if (!ui.welcome.hidden) return;
        ui.welcome.hidden = false;
        ui.welcome.querySelector('.choice')?.focus();
        return;
    }

    if (ui.welcome.hidden) return;
    ui.welcome.classList.add('leaving');
...
function render(next) {
    ...
    setWelcome(!next.languageChosen);
